...
def readColmapSceneInfo(path, images, eval, llffhold=11):
    ...
    N_TEST = 29
    N_TEST = min(N_TEST, len(cam_infos))  # safety

    train_cam_infos = cam_infos[:-N_TEST] if N_TEST > 0 else cam_infos
    test_cam_infos  = cam_infos[-N_TEST:] if N_TEST > 0 else []
    ...
    #ply_path = os.path.join(path, "sparse/0/")
    ...
    #ply_path = os.path.join(path, "sparse/0/points3D_okvis.ply")
