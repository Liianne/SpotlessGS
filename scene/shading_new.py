...
class ShadingModel(nn.Module):
    ...
    def set_optimizer(self)->None:
        l = [
            ...
            {'params': list(self.brdf.parameters()), 'lr': 0.001, "name": "brdf_mlp"},
    ...
    def forward(self, pts: Tensor, camera: Camera, albedos: Tensor, normals: Tensor)-> Tensor:
        ...
        #incident_light = self.light(pts_in_cam.unsqueeze(0)).transpose(0, 1)  # shape: (100000, 1)

        # Reshape incident_light to (100000, 1, 1)
        #incident_light = incident_light.unsqueeze(-1)  # shape: (100000, 1, 1)

        # Broadcast incident_light to match the shape of reflectance
        #incident_light = incident_light.expand(-1, 3, -1)  # shape: (100000, 3, 1)

        #reflectance = self.brdf(view_dir, normals, light_dir, F.softplus(albedos))[..., None]  # Pass albedo to BRDF
        ...
        # Compute ambient (indirect) light
        #ambient_light = torch.matmul(sh_basis, self.sh_coeffs)  # Shape: (N, 3)
        # sh_values_normal, sh_values_light = self.eval_sh_basis(normals, light_dir)

        # device = self.sh_coeffs.device
        # #print("device is: ",device)
        # sh_values_normal = sh_values_normal.to(device)
        # #sh_values_light = sh_values_light.to(device)

        # #sh_values_normal = sh_values_normal
        # #sh_values_light = sh_values_light.real
        ...
        reflectance = self.brdf(torch.cat([view_dir, normals, light_dir], dim=-1))[..., None] # n*1

        #reflectance = self.brdf(torch.cat([view_dir, normals, light_dir, albedos], dim=-1))[..., None] # n*1
        #reflectance = self.brdf(view_dir, normals, light_dir)[..., None]

        reflectance = reflectance.squeeze(-1)
        
        #reflected_light = incident_light * reflectance
        #reflected_light = reflected_light.squeeze(-1)  # shape: (100000, 3)

        incident_light = self.light(pts_in_cam.unsqueeze(0)).transpose(0,1)
        
        assert not torch.isnan(pts_in_cam).any(), "NaNs in pts_in_cam"
        assert not torch.isnan(normals).any(), "NaNs in normals"
        assert not torch.isnan(light_dir).any(), "NaNs in light_dir"
        assert not torch.isnan(view_dir).any(), "NaNs in view_dir"
        assert not torch.isnan(albedos).any(), "NaNs in albedos"
        assert not torch.isnan(incident_light).any(), "NaNs in incident_light"
        assert not torch.isnan(ambient_light).any(), "NaNs in ambient_light"
        assert not torch.isnan(reflectance).any(), "NaNs in reflectance"

           

        # print(f"The size of reflectance {reflectance.size()}")
        # print(f"The size of incident_light {incident_light.size()}")
       ...
       # reflected_light = (incident_light + ambient_light) * reflectance

        #reflected_light = F.softplus(albedos) * incident_light * reflectance
        
        #print("ambient light: ", self.ambient_light)
        #print("reflected light: ", reflected_light)
